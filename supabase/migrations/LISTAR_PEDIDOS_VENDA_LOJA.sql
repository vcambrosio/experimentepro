-- LISTAR APENAS PEDIDOS DO TIPO venda_loja
...
SELECT 
    ...
    p.status_pagamento,
    CASE 
        WHEN p.orcamento_id IS NOT NULL OR p.setor_id IS NOT NULL THEN 'DEVERIA SER evento_cesta'
        ELSE 'OK'
    END as analise
FROM public.pedidos p
LEFT JOIN public.clientes c ON p.cliente_id = c.id
LEFT JOIN public.setores_cliente s ON p.setor_id = s.id
WHERE p.tipo_pedido = 'venda_loja'
